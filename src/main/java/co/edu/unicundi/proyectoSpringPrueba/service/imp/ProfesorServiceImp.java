...
public class ProfesorServiceImp implements IProfesorService {
	...
	public Profesor traerProfesorPorcedula(String cedula) throws ModelNotFoundException {
		...
		// if(profesor==null){
		// throw new ModelNotFoundException("La cedula solicitada no se encuentra
		// registrada");
		// }
		return profesor;
	}
	...
	@Override
	public Page<Profesor> traerProfesores(int pagina, int registros, String orden, String tipo)
		...
		if ((validarOrden(orden))) {
			...
			if (tipo.equals("DSD")) {
				...
				listaOrdenada = this.profesorRepo
						.findAll(PageRequest.of(pagina, registros, Sort.Direction.DESC, orden));
			}
		}
	...
	public Profesor traerPorId(Integer id) throws ModelNotFoundException {
		Profesor profesor = profesorRepo.findById(id)
				.orElseThrow(() -> new ModelNotFoundException("el registro con el id solicitado no existe"));
		return profesor;
	}

	public boolean validarOrden(String orden) {
		System.out.print("Entre a validar orden");
		if (((orden.equals("nombre")) || (orden.equals("apellido")) || (orden.equals("cedula"))
				|| (orden.equals("edad")))) {
